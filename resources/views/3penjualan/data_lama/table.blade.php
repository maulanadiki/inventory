    ...
    <div class="col-md-4">
        <nav aria-label="breadcrumb">
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item active fs-5">Data Penjualan</li>
            </ul>
        </nav>
    </div>
    <div class="col-md-8 d-flex justify-content-end">
        <a href="{{route('penjualan.buat') }}" onclick="deletesession()" class="btn btn-primary me-5">
...
<div class="row justify-content-center ">
    <div class="col-md-11 bg-light border rounded-3  mt-3 shadow">
        ...
        <table class="table table-hover table-striped" id="penjualan">
            ...
            <tbody>
            @foreach($csm as $data)
            <tr style="cursor: pointer;" align="center" style="cursor: pointer;" onclick="nilai('{{$data->invoice }}','{{$data-> nama_pelanggan}}','{{$data->alamat_pelanggan }}','{{$data-> telp}}','{{$data-> tgl_jual}}','{{$data->market_place }}','{{$data->grandtotal }}','{{$data->email }}','{{$data->bukti_pembelian}}')">
                <th style=" text-align:center;" data-bs-toggle="modal" data-bs-target="#detailsell">{{$loop->iteration}}</th>
                ...
                <td id="cust" data-bs-toggle="modal" data-bs-target="#detailsell">{{$data->nama_pelanggan}}</td>
                <td id="grandtotal" data-bs-toggle="modal" data-bs-target="#detailsell">Rp. {{  number_format($data->grandtotal) }}</td>
                <td>  @if(auth()->user()->level== 1 || auth()->user()->level== 2 )
                        @if ( $data->stat_keluar == "Pending" )
                        <form method="get" action="{{ route('penjualan.barang_keluar',['Approved',$data->invoice])}}">
                            @csrf
                            <div class="dropdown">
                                <button class="btn btn-warning text-dark dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    PENDING
                                </button>
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            @endforeach
                                                <button class="dropdown-item" type="submit" > Approved</button></li>
                                            <li><a class="dropdown-item" href="{{ route('penjualan.barang_keluar', ['Rejected',$data->invoice])}}"  >Rejected</a></li>
                                        </ul>
                    ...
                    @else
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="{{ route('penjualan.barang_keluar.hapus', $data->invoice ) }}">Hapus</a></li>
                            </ul>
                        </div>
                        @endif
                    @endif
                ...
                <td>
                    @if($data->stat_keluar == "Approved")
                        @if( $data->stat_sell == "Pending")
                           <button class="btn btn-warning text-dark"  data-bs-toggle="modal" data-bs-target="#uploadresi" onclick="upload('{{$data->invoice}}')" >Upload Resi</button>
                        @elseif ($data->stat_sell == "Approved")
                            <a href ="" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#buktiresi" onClick="resi('{{$data->bukti_resi }}') ">Cek Resi</a>
                        @else
...
<!-- modal upload resi -->
<div class="modal fade" id="uploadresi" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
        <form  action="{{ route('penjualan.simpan_resi')}}" enctype="multipart/form-data" method="post" >
            ...
            <div class="modal-body1">
                    ...
                    <input class="form-control mt-3" type="file" id="formFile" name="resi" accept="image/png, image/gif, image/jpeg" required>
            </div>
            <div class="modal-footer">
                <button type="submit" class="btn btn-primary mb-3" style="width: 110px;"> <i class="fa fa-floppy-o" aria-hidden="true"></i> Simpan</button>
            </div>
...
<!-- bukti resi -->
<div class="modal fade" id="buktiresi" height="300" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable modal-xl">
    <div class="modal-content">
      <div class="modal-header">
